...
@section('content')
    <x-util.card title="{{ $data['title'] }}">
        <x-form.base url="{{ $data['url'] }}" method="POST">
            ...
            <x-form.input name="start_at" label="Start At" placeholder="choose when the test start"
                value="{{ $subjectTest->name ?? '' }}" />
            <x-form.input name="end_at" label="End At" placeholder="choose when the test end"
                value="{{ $subjectTest->name ?? '' }}" />
